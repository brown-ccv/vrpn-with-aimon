...
int vrpn_Generic_Server_Object::setup_Lenovo_G120(char * & pch, char * line, FILE * config_file) {
  char s2 [LINESIZE];

  next();
  if (sscanf(pch,"%511s",s2)!=1) {
    fprintf(stderr,"Bad Lenovo G120 line: %s\n",line);
    return -1;
  }

  // Open the Lenovo G120
  // Make sure there's room for a new button
  if (num_buttons >= VRPN_GSO_MAX_BUTTONS) {
    fprintf(stderr,"vrpn_LenovoG120: Too many buttons in config file");
    return -1;
  }
  ...
  if ( (buttons[num_buttons] = new vrpn_LenovoG120(s2, connection)) == NULL ) {
    fprintf(stderr,"Can't create new vrpn_LenovoG120\n");
     return -1;
  } else {
    num_buttons++;
  }
...
int vrpn_Generic_Server_Object::setup_Aimon_Elite (char * & pch, char * line, FILE * config_file)
{
  char s2 [LINESIZE];

  next();
  if (sscanf (pch, "%511s", s2) != 1) {
    fprintf (stderr, "Bad Aimon_Elite line: %s\n", line);
    return -1;
  }

  // Open the Aimon_Elite
  // Make sure there's room for a new button
  if (num_buttons >= VRPN_GSO_MAX_BUTTONS) {
    fprintf (stderr, "vrpn_Aimon_Elite: Too many buttons in config file");
    return -1;
  }

  // Open the button
  if (verbose) {
    printf ("Opening vrpn_Aimon_Elite %s\n", s2);
  }
  if ( (buttons[num_buttons] = new vrpn_Aimon_Elite (s2, connection)) == NULL) {
    fprintf (stderr, "Can't create new vrpn_Aimon_Elite\n");
    return -1;
  } else {
    num_buttons++;
  }
...
#define isit(s) !strcmp(pch=strtok(scrap," \t"),s)
	...
	CHECK(setup_DevInput);
      ...
      } else if (isit ("vrpn_Lenovo_G120")) {
        CHECK (setup_Lenovo_G120);
      } else if (isit ("vrpn_Aimon_Elite")) {
        CHECK (setup_Aimon_Elite);
